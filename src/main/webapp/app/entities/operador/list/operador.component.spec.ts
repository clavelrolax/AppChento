...
import { sampleWithRequiredData } from '../operador.test-samples';
...
describe('Operador Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.operadors()[0]).toEqual(expect.objectContaining({ id: 6338 }));
  });

  describe('trackId', () => {
    it('Should forward to operadorService', () => {
      const entity = { id: 6338 };
      jest.spyOn(service, 'getOperadorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getOperadorIdentifier).toHaveBeenCalledWith(entity);
