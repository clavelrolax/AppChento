...
@Component({
  ...
  templateUrl: './datos-imagen.component.html',
  imports: [RouterModule, FormsModule, SharedModule, SortDirective, SortByDirective],
})
export class DatosImagenComponent implements OnInit {
  subscription: Subscription | null = null;
  datosImagens = signal<IDatosImagen[]>([]);
  isLoading = false;
  ...
  delete(datosImagen: IDatosImagen): void {
    const modalRef = this.modalService.open(DatosImagenDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.datosImagen = datosImagen;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.datosImagens.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IDatosImagen[]): IDatosImagen[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IDatosImagen[] | null): IDatosImagen[] {
    return data ?? [];
  }
